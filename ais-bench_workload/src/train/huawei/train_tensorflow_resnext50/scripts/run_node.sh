...
{
    [[ $RANK_SIZE -eq 1 ]] && CONFIG_FILE_32bs_1p_host || CONFIG_FILE=res50_32bs_8p_host
    ln -sf $WORK_PATH/code/code/resnext50_train/configs ../
    sed -i "s|'num_epochs':.* |'num_epochs': 3, |g" $WORK_PATH/code/code/resnext50_train/configs/$CONFIG_FILE.py
    sed -i "s|'rank_size':.* |'rank_size': $RANK_SIZE, |g" $WORK_PATH/code/code/resnext50_train/configs/$CONFIG_FILE.py
    mkdir -p /data/
    ln -sf $TRAIN_DATA_PATH /data/imagenet_TF

    train_run_cmd="${PYTHON_COMMAND} -u $WORK_PATH/code/code/resnext50_train/mains/res50.py \
...
function node_init()
{
    export PYTHONPATH=$PYTHONPATH:$WORK_PATH:$WORK_PATH/code/code/resnext50_train
