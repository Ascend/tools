...
check_env()
{
    ...
    : "${TRAINING_FILE_PATTERN?TRAINING_FILE_PATTERN not set}"
    : "${RESNET_CHECKPOINT?RESNET_CHECKPOINT not set}"
    : "${VALIDATION_FILE_PATTERN?VALIDATION_FILE_PATTERN not set}"
    : "${VAL_JSON_FILE?VAL_JSON_FILE not set}"

    # check env of each node
    cmd="export WORK_PATH=$WORK_PATH;
        bash -x $WORK_PATH/run_node.sh check ${WORK_PATH}/config/$CONFIG_FILE"
...
run_train()
{
    ...
    cmd="export WORK_PATH=$WORK_PATH;
        ...
        bash -x $WORK_PATH/run_node.sh train"
