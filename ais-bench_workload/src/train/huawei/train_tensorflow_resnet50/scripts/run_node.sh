...
{
    train_run_cmd="${PYTHON_COMMAND} -u $WORK_PATH/code/official/r1/resnet/imagenet_main.py  \
    --resnet_size=$RESNET_SIZE --resnet_version=1  \
    --epochs_between_evals=$EPOCH_SIZE --hooks=ExamplesPerSecondHook \
    --train_epochs=$EPOCH_SIZE \
    --batch_size=$BATCH_SIZE --data_dir=${TRAIN_DATA_PATH} --model_dir=$RUN_PATH/"
    return 0
}
