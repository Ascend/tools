...
set_config_env()
{
    ...
    source $CUR_PATH/config/config_imagenet2012.sh
}

# 环境变量检查
check_env()
{
    : "${EPOCH_SIZE?EPOCH_SIZE not set}"
}

# 文件路径和环境依赖检查
check_sys()
{
    check_path_valid $TRAIN_DATA_PATH || { logger_Warn "TRAIN_DATA_PATH:${TRAIN_DATA_PATH} not valid path" ; return $ret_invalid_args; }
    logger_Debug "train path valid"
    #check_path_valid $TEST_DATA_PATH || { logger_Warn "TEST_DATA_PATH:${TEST_DATA_PATH} not valid path" ; return $ret_invalid_args; }
    #logger_Debug "test path valid"
