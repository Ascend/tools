...
def process(image_path, out_path):
    ...
    flbin = os.path.join(out_path, image_name.split('.')[0] + ".bin")
    imgSrc = cv2.imread(image_path)
    img_process = process_image(imgSrc, 416)
    imgDst = np.array(img_process)
    imgDst = np.expand_dims(imgDst, axis=0)
    imgDst = imgDst.astype("uint8")
...
if __name__ == "__main__":
    ...
    for image_name in images:
        ...
        process(os.path.join(in_path, image_name), out_path)
