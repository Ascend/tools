...
check_sys()
{
    ...
    check_path_valid "${DATASET_PATH}" || { logger_Warn "DATASET_PATH:${DATASET_PATH} not valid path" ; return 1; }
    ...
    check_path_valid ${CACHE_PATH} || { logger_Warn "CACHE_PATH:${CACHE_PATH} not valid" ; return 1; }
    logger_Debug "CACHE_PATH file valid"
...
exec_inference()
{
    ...
    infer_run_cmd="$PYTHON_COMMAND -u $WORK_PATH/main.py --profile=$PROFILE --dataset_path=$DATASET_PATH \
            --model $MODEL_PATH --cache_path $CACHE_PATH --query_arrival_mode=$QUERY_ARRIVAL_MODE"
    ...
    [ "$DEBUG" == "True" ] && infer_run_cmd="${infer_run_cmd} --debug"
