...
class Compare(ToolObject):
    ...
    def vector_summary(self):
        ...
        for file_name in self.vector_compare_result:
            ...
            table = Table(title=file_name)
            table.add_column('TensorIndex', overflow='fold')
            table.add_column('CosSim')
            table.add_column('MaxAbsErr')
            table.add_column('ARE')
            table.add_column('RED')
            table.add_column('KLD')
            table.add_column('StandardDeviation')
            for item in items:
            ...
            print("[CosSim: CosineSimilarity] [MaxAbs: MaxAbsoluteError] [ARE: MaxAbsoluteError] "
    ...
    def compare_data(self, left, right, rl=0.001, al=0.001, print_n=20):
        ...
        err_table = self._create_diff_table("Error Item Table (rl= %s, al= %s)" % (rl, al))
        top_table = self._create_diff_table("Top Item Table")
        ...
        print(Columns([err_table, top_table]))
