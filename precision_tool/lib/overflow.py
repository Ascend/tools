...
class Overflow(ToolObject):
    ...
    def check(self):
        ...
        for op_name in self.overflow_ops:
            ...
            self.decode_overflow_info(file_info)
            self.parse_overflow_info(file_info)
        LOG.info("[Overflow] Checked failed. find [%d] overflow node!" % len(self.overflow_ops))
    ...
    '''
    def _parse_overflow_files_dsp(self):
        overflow_debug_files = {}
        current_dir = ''
        for file_name in self.overflow_dump_files:
            file_info = self.overflow_dump_files[file_name]
            op_name = file_info['op_name']
            ...
            if op_name not in self.overflow_ops:
                self.overflow_ops[op_name] = {}
            if current_dir != file_info['dir_path']:
                overflow_debug_files = util.list_overflow_debug_files(file_info['dir_path'])
            for debug_file in overflow_debug_files.values():
                self._update_debug_task_id(debug_file)
                if file_info['task_id'] == debug_file['task_id']:
                    file_info['debug_file'] = debug_file
                    break
            current_dir = file_info['dir_path']
            self.overflow_ops[op_name][timestamp] = file_info

    @staticmethod
    def _update_debug_task_id(debug_file):
        ...
        if len(debug_decode_files) == 0:
            ...
            debug_decode_files = util.list_debug_decode_files(cfg.DUMP_FILES_OVERFLOW_DECODE, debug_name)
        with open(list(debug_decode_files.values())[0]['path'], 'r') as f:
            overflow_json = json.load(f)
            for item in overflow_json.values():
                if item['status'] != 0 and item['task_id'] != int(debug_file['task_id']):
                    debug_file['task_id'] = str(item['task_id'])
    '''

    def decode_overflow_info(self, file_info):
    ...
    def parse_overflow_info(self, file_info):
        """"""
        ...
        for debug_decode_file in debug_decode_files.values():
            with open(debug_decode_file['path'], 'r') as f:
                overflow_json = json.load(f)
                util.print_panel(self.json_summary(overflow_json, file_info))
                # self._parse_overflow_json(overflow_json, file_info)

    def json_summary(self, json_txt, file_info):
    ...
    '''
    def _parse_overflow_json(self, json_txt, file_info):
        """"""
        task_id = -1
        if 'AI Core' in json_txt and json_txt['AI Core']["status"] != 0:
            detail = json_txt['AI Core']
            task_id = detail['task_id']
            LOG.error('[Overflow][AiCore Overflow][task_id:%d][status:%s][%s]' % (
                task_id, detail['status'], self._decode_ai_core_status(detail['status'])))
        if 'DHA Atomic Add' in json_txt and json_txt['DHA Atomic Add']['status'] > 0:
            detail = json_txt['DHA Atomic Add']
            task_id = detail['task_id']
            LOG.error('[Overflow][DHA Atomic Add Overflow][task_id:%d][status:%s][%s]' % (
                task_id, detail['status'], detail['status']))
        if 'L2 Atomic Add' in json_txt and json_txt['L2 Atomic Add']['status'] > 0:
            detail = json_txt['L2 Atomic Add']
            task_id = detail['task_id']
            LOG.error('[Overflow][L2 Atomic Add Overflow][task_id:%d][status:%s][%s]' % (
                task_id, detail['status'], detail['status']))
        LOG.error('[Overflow][Op][%s][%s]' % (file_info['op_type'], file_info['op_name']))
        # LOG.error('[Overflow][Files]%s' % filter(lambda x: 'path' in x, file_info['dump_decode_files'].values()))
        for dump_decode_file in file_info['dump_decode_files'].values():
            file_name = dump_decode_file['path']
            util.npy_summary(file_name)
            util.convert_npy_to_txt(file_name)
            LOG.error("[Overflow][Detail File]%s.txt" % file_name)
    '''
