...
flags.DEFINE_integer('device_count', 1, 'number of device')
...
# Data processing
###############################################################################
def get_filenames(is_training, data_dir, test_mode=False):
    """Return filenames for dataset."""
    if test_mode == True:
        tf.compat.v1.logging.info('test mode, data_dir is : %s', data_dir)
        return [
...
def parse_record(raw_record, is_training, dtype):
    ...
    # add by wx933141
    # resnet_dedicated_log.logger1.info("namespace:%s,time_ms:%d, event_type:pre_process_event,root_dir:%s" % ())
    # resnet_dedicated_log.logger1.info("namespace:%s,time_ms:%d,event_type:init_start, root_dir:%s" % ())
    ...
    # resnet_dedicated_log.logger1.info("namespace:%s,time_ms:%d,event_type:init_stop, root_dir:%s" % (resnet_dedicated_log.work_num, resnet_dedicated_log.datatime, resnet_dedicated_log.root_dir))
    return image, label
...
def input_fn(is_training,
    ...
    """
    filenames = get_filenames(is_training, data_dir, test_mode=test_mode)
    dataset = tf.data.Dataset.from_tensor_slices(filenames)

    if input_context and not test_mode:
        ############## npu modify begin #############
        dataset = dataset.shard(rank_size, rank_id)
        ############## npu modify end ###############

    if is_training and not test_mode:
...
def imagenet_model_fn(features, labels, mode, params):
    ...
    learning_rate_fn = resnet_run_loop.learning_rate_with_decay(
        ...
        cosine_lr=params['cosine_lr'], train_epochs=params['train_epochs'],
    )
...
def run_imagenet(flags_obj):
    ...
    Returns:
      Dict of results of the run.  Contains the keys `eval_results` and
        ...
        training.
    """
    if flags_obj.random_seed >= 0:
        SEED = flags_obj.random_seed
    else:
        SEED = 0
    ...
    if flags_obj.use_synthetic_data == True:
        tf.compat.v1.logging.info('using synthetic data.')
        input_function = get_synth_input_fn(flags_core.get_tf_dtype(flags_obj))
    else:
...
def main(_):
    ...
    with logger.benchmark_context(flags.FLAGS):
        flags.FLAGS.model_dir = os.getenv('MODEL_CKPT_PATH')
