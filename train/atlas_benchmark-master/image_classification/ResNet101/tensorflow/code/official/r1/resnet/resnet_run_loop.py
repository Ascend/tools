...
tf.compat.v1.logging.info('setting random_seed: %d', SEED)
...
def learning_rate_with_decay(
    ...
    base_lr=0.1, warmup=False, cosine_lr=False, train_epochs=90):
  ...
  tf.compat.v1.logging.info('base_lr: %.4f', float(base_lr))
  tf.compat.v1.logging.info('initial_learning_rate: %.4f', float(initial_learning_rate))
  tf.compat.v1.logging.info('batches_per_epoch: %d', int(batches_per_epoch))
  tf.compat.v1.logging.info('lr boundaries: %s', ','.join(str(i) for i in boundaries))
  tf.compat.v1.logging.info('lr vals: %s', ','.join(str(i) for i in vals))

  tf.compat.v1.logging.info('warmup: %d', int(warmup))
  tf.compat.v1.logging.info('cosine_lr: %d', int(cosine_lr))
  tf.compat.v1.logging.info('train_epochs: %d', int(train_epochs))
  ...
  def learning_rate_fn(global_step):
    ...
    ############## npu modify begin #############
    if cosine_lr:
      total_steps = int(train_epochs * batches_per_epoch)
      warmup_steps = int(batches_per_epoch * 5 * int(warmup))
...
def resnet_main(
  ...
  ############## npu modify begin #############
  # Creates a `NPURunConfig` that checkpoints every 115G00 steps
  iterations_per_loop=flags_obj.iterations_per_loop
  run_config = NPURunConfig(
        model_dir=flags_obj.model_dir,
        tf_random_seed=flags_obj.random_seed if flags_obj.random_seed>=0 else 0,
  ...
  classifier = NPUEstimator(
      ...
      params={
          ...
          'loss_scale': flags_core.get_loss_scale(flags_obj,
                                                  default_for_fp16=512),
          'dtype': flags_core.get_tf_dtype(flags_obj),
          'fine_tune': flags_obj.fine_tune,
          'num_workers': num_workers,
          'num_gpus' : flags_core.get_num_gpus(flags_obj),
          'base_lr': flags_obj.base_lr,
          'cosine_lr': flags_obj.cosine_lr,
  ...
  def input_fn_train(num_epochs, input_context=None, rank_size=None, rank_id=None):
    ############## npu modify begin #############
  ...
  if use_train_and_evaluate:
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda input_context=None: input_fn_train(
            train_epochs, input_context=input_context, rank_size=rank_size, rank_id=rank_id),
        hooks=train_hooks,
    ...
    if train_epochs == 0:
      ...
      ############## npu modify begin #############
      tf.compat.v1.logging.info('schedule: %s', ','.join([str(i) for i in schedule]))
      ############## npu modify end #############
        ...
        if flags_obj.max_train_steps is None:
            classifier.train(
                input_fn=lambda input_context=True: input_fn_train(
                    num_train_epochs, input_context=input_context, rank_size=rank_size, rank_id=rank_id),
                hooks=train_hooks,
                steps=(num_images['train']/flags_obj.batch_size/rank_size)*num_train_epochs // iterations_per_loop * iterations_per_loop )  # step control func.2
        else:
            classifier.train(
                input_fn=lambda input_context=True: input_fn_train(
                    num_train_epochs, input_context=input_context, rank_size=rank_size, rank_id=rank_id),
                ...
                max_steps=flags_obj.max_train_steps)
        ############## npu modify end ###############
        date_time_end = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
      if cycle_index == len(schedule) - 1 and flags_obj.test_mode == False:
        ...
        eval_results = classifier.evaluate(input_fn=input_fn_eval,
                                           steps=num_images['validation']//flags_obj.batch_size)  # step control func.1
  ...
  stats = {}
  stats['eval_results'] = eval_results if flags_obj.test_mode == False else []
...
def define_resnet_flags(resnet_size_choices=None, dynamic_loss_scale=False,
  ...
  ##############NPU modify#######################
  flags.DEFINE_float(
      name='base_lr', default=0.0,
      help=flags_core.help_wrap(
          'base learning rate in training.'))
  flags.DEFINE_bool(
      name='cosine_lr', default=False,
      help=flags_core.help_wrap(
          'Enable Cosine Learnin_rate for training.'))
  flags.DEFINE_bool(
      name='test_mode', default=False,
      help=flags_core.help_wrap(
          'turn to test mode.'))
  flags.DEFINE_integer(
      name='iterations_per_loop', default=10,
      help=flags_core.help_wrap(
          'setting iterations_per_loop, this config will affect performance.'))
  flags.DEFINE_integer(
      name='random_seed', default=-1,
      help=flags_core.help_wrap(
          'setting global random_seed to make result stable.'))
  ##############NPU modify end#######################
