...
export REMARK_LOG_FILE=hw_resnet101.log

mkdir -p ${currentDir%train*}/train/result/tf_resnet101/training_job_${currtime}/
export train_job_dir=${currentDir%train*}/train/result/tf_resnet101/training_job_${currtime}/
...
if [ x"$6" == x"True" ];then
    export CLUSTER=True
    echo "run cluster"
    python3.7 ${currentDir}/code/official/r1/resnet/imagenet_main.py \
        --device_count=${rank_size} \
        --resnet_size=101 \
        ...
        --data_dir=${data_url} \
        --model_dir=${MODEL_CKPT_PATH}  > ${train_job_dir}/train_${device_id}.log 2>&1
else
    python3.7 ${currentDir}/code/official/r1/resnet/imagenet_main.py \
        --device_count=${rank_size} \
        --resnet_size=101 \
...
if [ $? -eq 0 ] ;then
    echo ":::ABK 1.0.0 resnet101 train success"
    echo ":::ABK 1.0.0 resnet101 train success" >> ${train_job_dir}/train_${device_id}.log
    echo ":::ABK 1.0.0 resnet101 train success" >> ${train_job_dir}/${device_id}/hw_resnet101.log
else
    echo ":::ABK 1.0.0 resnet101 train failed"
    echo ":::ABK 1.0.0 resnet101 train failed" >> ${train_job_dir}/train_${device_id}.log
    echo ":::ABK 1.0.0 resnet101 train failed" >> ${train_job_dir}/${device_id}/hw_resnet101.log
fi
...
echo ":::ABK 1.0.0 resnet101 train total time ${hour}:${min}:${sec}"
echo ":::ABK 1.0.0 resnet101 train total time ${hour}:${min}:${sec}" >> ${train_job_dir}/${device_id}/hw_resnet101.log
